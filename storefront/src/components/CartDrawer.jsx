...
export default function CartDrawer({ items, onClose, onQuantity, onCheckout }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/50" onClick={onClose}>
      <aside
        className="absolute left-0 top-0 h-full w-full max-w-md bg-white
                   flex flex-col shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-center gap-2 p-4 border-b border-slate-200">
        ...
        <div className="flex-1 overflow-auto p-4 space-y-3">
          {items.length === 0 && (
            <p className="text-center text-slate-500 py-16">السلة فارغة</p>
          )}

          {items.map((item, index) => (
            <div
              key={`${item.id}|${item.size}|${item.color}`}
              className="flex gap-3 items-center border border-slate-200 rounded-xl p-3"
            >
        ...
        <footer className="p-4 border-t border-slate-200 space-y-3">
